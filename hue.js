...
let address = 'http://172.22.151.183/';       // IP address of the Hue hub
let username = '1IVlFMzSgaLyWNiF8RAiI4bVyHwJgMBReCz0DkQ5';      // username on the hub
let requestUrl = 'http://ipaddress/api/';  // generic hub URL
let lightNumber = 8;    // number of the light to control
let lightState = {      // JSON with the state of the light
    on: true,
    bri: 200
};
const fadeUpLights = [1, 8, 4, 7]; // list of lights to fade up
const fadeDownLights = [9, 10];    // list of lights to fade down
// a div for responses from the Hue hub:
let responseDiv;
// UI elements in the page:
let slider, onButton, statusButton, newuserButton;
let fadeInterval = null;  // Move this to the top!
...
function setup() {
    ...
    document.getElementById("FadeUpDown").addEventListener("click", () => {
        fadeLights("updown");
    });
    ...
    document.getElementById("AutoFade").addEventListener("click", startAutoSwitch);
...
function setCreds() {
    ...
    address = "172.22.151.183";

    const userField = document.getElementById("username");
    username = "1IVlFMzSgaLyWNiF8RAiI4bVyHwJgMBReCz0DkQ5";
...
let currentDirection = 'updown'; // can be 'updown' or 'downup'
let autoSwitchInterval = null;
const fadeDuration = 60; // in deciseconds = 7 seconds

function fadeLights(direction) {
    const briUp = 250;
    const briDown = 1;

    const stateUp = { on: true, bri: briUp, transitiontime: fadeDuration };
    const stateDown = { on: true, bri: briDown, transitiontime: fadeDuration };

    if (direction === 'updown') {
        fadeUpLights.forEach(lightNum => {
            sendRequest(`lights/${lightNum}/state/`, 'PUT', stateUp);
        });
        fadeDownLights.forEach(lightNum => {
            sendRequest(`lights/${lightNum}/state/`, 'PUT', stateDown);
        });
    } else {
        fadeUpLights.forEach(lightNum => {
            sendRequest(`lights/${lightNum}/state/`, 'PUT', stateDown);
...
function startAutoSwitch() {
    fadeLights(currentDirection); // initial fade

    // Set an interval for the fade switch
    autoSwitchInterval = setInterval(() => {
        setTimeout(() => {
            // Switch between 'updown' and 'downup' every 4 seconds
            currentDirection = currentDirection === 'updown' ? 'downup' : 'updown';
            fadeLights(currentDirection);
        }, 3000); // 4000ms = 4 seconds delay before switching
    }, 6000); // The interval for switching after the fade duration (7s)
}
